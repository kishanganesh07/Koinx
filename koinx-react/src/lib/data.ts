export interface Holding {
  id: string; // unique key (coin + index)
  coin: string;
  coinName: string;
  logo: string;
  currentPrice: number;
...
// Simulate async API calls with 400ms delay
export const fetchHoldings = (): Promise<Holding[]> =>
  new Promise((resolve) => setTimeout(() => resolve(HOLDINGS_DATA), 400));

export const fetchCapitalGains = (): Promise<CapitalGainsData> =>
  new Promise((resolve) => setTimeout(() => resolve(CAPITAL_GAINS_DATA), 400));

// Capital Gains API - exact from requirements
const CAPITAL_GAINS_DATA: CapitalGainsData = {
  capitalGains: {
...
// Holdings API - exact from requirements (with unique id added)
const RAW_HOLDINGS = [
  {
    coin: "USDC",
    coinName: "USDC",
...
// Sort by gain (descending) and add unique id
const HOLDINGS_DATA: Holding[] = RAW_HOLDINGS.map((h, i) => ({
  ...h,
  id: `${h.coin}-${i}`,
})).sort((a, b) => {
